...
namespace ML
{
    public class ConfigFile
    {
        ...
        private bool Load(string path, bool ignoreUselessLines)
        {
            ...
            if (!File.Exists(path))
            {
                Debug.LogWarning("Error loading config file at: " + path + ", try load in Resources folder");

                TextAsset contents = Resources.Load<TextAsset>(path);
                if (contents != null)
                {
                    streamReader = new StreamReader(new MemoryStream(contents.bytes));
                }
                else
                {
                    Debug.LogWarning("Error loading config file in Resources folder.");
                }
            }
            else
            {
            ...
            while (streamReader.Peek() != -1)
            {
                ...
                {
                    bool flag = text3.Length > 0 && (text3[0] == ';' || text3[0] == '#');
