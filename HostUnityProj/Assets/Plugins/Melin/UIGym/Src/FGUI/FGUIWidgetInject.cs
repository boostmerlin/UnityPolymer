...
namespace ML.UI
{
    public class FGUIWidgetInject
    {
        ...
        public static void Inject(object view, GObject componentRoot)
        {
            ...
            {
                Log.ML.PrintWarning("FGUIWidgetInject.Inject, null componentRoot or view");
                return;
            }
        ...
        static GObject findWidget(GObject component, string fieldName, UIWidgetAttribute attribute)
        {
            ...
            if (!string.IsNullOrEmpty(attribute.FullName))
            {
                ...
                for (int i = 0; i < Names.Length; i++)
                {
                    ...
                    if (go == null)
                    {
                        Log.ML.PrintWarning("Can't find component name={0} on path: {1}, field is:{2}", name, attribute.FullName, fieldName);
                        return null;
                    }
                    obj = go;
                }
            ...
            else
            {
                ...
                if (obj == null)
                {
                    Log.ML.PrintWarning("Can't find component by fieldname={0}", fieldName);
                    return null;
                }
            }
            return obj;
