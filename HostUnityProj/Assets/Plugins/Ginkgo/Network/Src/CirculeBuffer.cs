...
namespace Ginkgo.Net
{
    ...
    public sealed class CirculeBuffer
    {
        ...
        {
            Log.Net.Print(string.Format("network! CirculeBuffer State, Init Cap: {0}, Incre: {1},  CurCap: {2}, Expandable: {3}, Upper: {4}, CurLen: {5}, ReadPos: {6}, WritePos: {7}", INITCAPACITY, INCREMENTSIZE, mCurrentCapacity, mExpandable, mUppderCapacity, mLength, mReadPos, mWritePos));

            Log.Net.Print("network! Data In Buffer: ");
        ...
        private void WriteInternal(byte[] src, int offset, int count)
        {
            ...
            mLength += count;
#if DEBUG_NET
            Log.Net.Print("After Write Buffer of Data: " + count);
            PrintBufferState();
#endif
        }
...
#if DEBUG_NET
            Log.Net.Print("network! After Read Buffer of Data: " + count);
            PrintBufferState();
#endif
        }
...
#if DEBUG_NET
            Log.Net.Print("Expand Buffer Of Blocks: " + blocksNum);
#endif
